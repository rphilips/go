...
func fileCi(cmd *cobra.Command, args []string) error {
	...
	if Fmsg == "" {
		Fmsg = qreport.Report(result, Fcargo.Error, Fjq, Fyaml, Funquote, Fsilent)
	}
	return nil
}
...
func preCi(cmd *cobra.Command, args []string) {
	...
	if errlist != nil || Fpayload == nil {
		Fmsg = qreport.Report(nil, qerror.ErrorSlice(errlist), Fjq, Fyaml, Funquote, Fsilent)
...
func storeRep(payload *qclient.Payload) (pcargo *qclient.Cargo) {
	...
	for version, qpaths := range versions {
		...
		results, errs := qsource.StoreList("install", version, qpaths, false, fmeta, fdata)
		if errs != nil {
