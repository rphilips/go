...
The file selection works in two steps:
    ...
    - the candidates are all the files in the arguments. If arguments are
	  directories, the files in these directories are added instead.
	  If the --recurse' flag is given,  also the files in the subdirectories
	  are added.
    - this candidate list is then filtered according to the version, 
	  ...
	  with a qpath in this list are accepted. If there is an *notinlist*,
	  then only candidates with a qpath not in that list are accepted.
	  
The *inlist* and the *notinlist* refer to files in the 'lists' subdirectory
