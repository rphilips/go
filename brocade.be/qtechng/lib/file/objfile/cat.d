...
macro getCatGenStatus($data, $cloi):
    ...
    $example: m4_getCatGenStatus(Array,"c:lvd:1345678")
    '''
...
macro chkCatIsbdNumber($xcheck, $nr, $nrtype):
    '''
    $synopsis: Controleert een isbn/issn/isbn13/ean nummer
    $xcheck: return waarde : 0/1/leeg: correct/niet correct/er is geen test
    $nr: kandidaat ISBN/ISSN/.. nummer
    $nrtype: type: isbn/issn/isbn13/ean/...
    $example: m4_chkCatIsbdNumber(error,"ISSN 8778-023X","issn")
    '''
    «s $xcheck='$$%CheckNr^bcawnr($nr,$nrtype)»
...
macro isCatObject($xis, $oloi, $staff, $mode="E"):
    ...
    $example: m4_isCatObject(return,"o:anet:134679")
...
macro getCatRelations($array, $cloi, $start, $max="", $opac="", $loop=0):
    '''
    ...
    $array: Array die de relaties bevat.
            Deze array bevat drie niveaus van subscripts:
             1. het relatietype.  Dit niveau heeft nog een subniveau met exact vier waarden:
    ...
    '''
    «d %GetRe^gbcat(.$array,$cloi,.$start,$max,$opac,$loop)»
...
macro getCatRecordDescriptionForStaff($array, $cloi, $staff="", $arlnk=MAlnk, $lg="", $isbd=0, $coll=1, $mask=0):
    '''
    ...
    $mask: 0 | 1 (indien 1, de Unicode code karakters worden in een <span> geplaatst
    $example: m4_getCatRecordDescriptionForStaff(RAresult,"c:lvd:238746",UDuser,RAlink,UDlg)
    '''
    «d %Entry^bcawisbd(.$array,$cloi,$staff,.$arlnk,$lg,$isbd,$coll,$mask)»
...
macro selCatOpacsAll($catsys, $index=1):
    '''
    $synopsis: Verwijder alle Opacs bij een een REGELWERK
    $catsys: Naam van het regelwerk
    $index: 0 (= indexeer niet) | 1 (= indexeer)
    $example: m4_selCatOpacsAll("lvd")
    '''
    «d %Overall^bcasopac($catsys,0,$index)»
...
macro delCatPkEmptyVolumes($return, $cloi, $is, $ploi, $test):
    ...
    $example: m4_delCatPkEmptyVolumes($return=«return», $cloi=«"c:ob:3456"», $is=«"OB-Hoboken"», $ploi=«"p:ob:5677"»)
...
macro delCatPkEmptyHoldings($cloi, $is, $test):
    ...
    $is: acroniem instelling
    $test: optioneel . indien 1, dan wordt enkel gecheckt of er zou geschrapt worden
    $example: m4_delCatPkEmptyHoldings($cloi=«"c:lvd:3456"», $is=«"UFSIA"»)
...
macro getCatObjectsInRecord($oloilist, $cloi, $lib="", $ploi=""):
    ...
    $example: m4_getCatObjectsInRecord(RAoll,"c:lvd:3","","p:lvd:1")
    $example: m4_getCatObjectsInRecord(RAoll,"c:lvd:3","SBA","p:lvd:1")
...
macro getCatUsbc($usbc, $cloi, $reflen=8, $base="TEIC"):
    ...
    $example: m4_getCatUsbc(usbc, "c:lvd:3")
    $example: m4_getCatUsbc(usbc, "c:lvd:3", 10)
...
macro getCatRecordLocalData($data, $cloi, $normalize=1, $contids):
    ...
    $example: m4_getCatRecordLocalData(RAcoid, "c:irua:89701")
...
macro setCatRecordLocalData($data, $cloi, $normalized=1, $change, $contids, $mode="batch"):
    ...
    $example: m4_setCatRecordLocalData(RAcoid, "c:irua:89701")
    $example: m4_setCatRecordLocalData(RAcoid, "c:irua:89701",normalized=0)
...
macro getCatObjectLocalData($data, $oloi, $normalize=1, $contids):
    ...
    $example: m4_getCatObjectLocalData(RAcoid, "o:lvd:1819800")
...
macro setCatObjectLocalData($data, $oloi, $normalized=1, $change, $contids, $mode="batch"):
    ...
    $contids: optioneel. Array van de vorm (content id). Bestaat deze array, dan worden enkel de hier vermelde contents weg geschreven/overschreven.
    $mode: "batch" of "screen" (in dat geval wordt UDerror eventueel gezet)
    $example: m4_setCatObjectLocalData(RAcoid, "o:lvd:1819800")
    $example: m4_setCatObjectLocalData(RAcoid, "o:lvd:1819800",normalized=0)
...
macro showCatPkObjectExtra($show, $user, $oloi, $env="html"):
    ...
    $example: m4_showCatPkObjectExtra(RDshow,"grobijns","o:lvd:1819800")
